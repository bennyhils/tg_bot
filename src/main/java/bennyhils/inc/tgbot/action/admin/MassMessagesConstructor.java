...
@Slf4j
public class MassMessagesConstructor implements Action {

    private final Properties properties;

    private final OutlineService outlineService = new OutlineService();

    public MassMessagesConstructor(Properties properties) {
    ...
    public BotApiMethod<?> handle(Update update) {
        ...
        FileEngine.writeMassMessageToFile(Instant.now().toEpochMilli(), "1", 0, 0, 0);
        List<MassMessage> allMassMessages = FileEngine.getAllMassMessages();
        return new SendMessage(
                update.getMessage().getChatId().toString(),
                "Вот три ваших последних массовых сообщения и статистика по ним: " + allMassMessages.size()
        );
